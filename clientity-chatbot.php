...
if ( ! defined( 'CLIENTITY_PLUGIN_VERSION' ) ) define( 'CLIENTITY_PLUGIN_VERSION', '1.6.0' );
...
function clientity_enqueue_scripts()
{
	...
	if( $show_widget )
	{
		...
		// Problem with JQMigrate:  https://es.wordpress.org/support/topic/jqmigrate-migrate-is-installed-version-1-4-1/
...
function clientity_register_settings() {
	...
	register_setting( 'clientity-settings-group', 'clientity_excluded_posts' , array('default' => "") );
}
...
?>			
			<h2 class="nav-tab-wrapper">
				<a href="#" onclick="on_tab_nav('general-settings')" id="nav-tab-general-settings" class="nav-tab nav-tab-active">
					<?= __( 'General settings', 'clientity-chatbot' ) ?>
				</a>
				<a href="#" onclick="on_tab_nav('excluded')" id="nav-tab-excluded"  class="nav-tab">
					<?= __( 'Excluded posts', 'clientity-chatbot' ) ?>
				</a>
				<a href="#" onclick="on_tab_nav('multilanguage')" id="nav-tab-multilanguage"  class="nav-tab">
					<?= __( 'Multilanguage', 'clientity-chatbot' ) ?>
				</a>				
			</h2>
			
			<div id="tab-general-settings" class="tab-content tab-content-active">
				<br>
	...
	*/ ?>
				...
				<p><?= __( "Note that the chat window will NOT be shown when you're logged into your Wordpress site.", 'clientity-chatbot' ) ?></p>
				</div>
				<br>
			...
			<div id="tab-multilanguage" class="tab-content">
				<br>
<?php
				
				$my_current_lang = true; // apply_filters( 'wpml_current_language', NULL );


				if( !$my_current_lang )
				{
					echo '<br>';
					echo __( "You don't have the <b>WPML Plugin</b> installed for multilingual support.", 'clientity-chatbot' );
					echo '<br><br>';
				}
				else
				{
					//echo "Lang: " . $my_current_lang . " - " . ICL_LANGUAGE_CODE;

					echo '<br>';
					echo __( "Multilanguage is not supported yet.", 'clientity-chatbot' );
					echo '<br>';
					echo __( "Check again in the next version of this plugin.", 'clientity-chatbot' );
					echo '<br><br>';
				}
?>				
			</div>
			...
			<div id="tab-excluded" class="tab-content">
				...
				<table class="form-table">
					<tbody>
						<tr>
							<th scope="row"><label for="clientity_excluded_posts"><?= __( 'Excluded Slugs or IDs', 'clientity-chatbot' ) ?></label></th>
...
<script>
	function on_tab_nav(tab)
	{
		jQuery(".tab-content").removeClass("tab-content-active");
		jQuery("#tab-"+tab).addClass("tab-content-active");
